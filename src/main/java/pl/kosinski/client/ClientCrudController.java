...
public class ClientCrudController {
    ...
    @PostMapping("/create")
    public String createClient(ClientInfoDto clientInfoDto, BindingResult result) {
        if (result.hasErrors()) {
            return "/clients/create";
        }
    ...
    public String updateClient(ClientInfoDto clientInfoDto, BindingResult result, @PathVariable long id) {
        if (result.hasErrors()) {
